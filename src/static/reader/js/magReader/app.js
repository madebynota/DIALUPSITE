(function(){
	...
	var State = Backbone.Model.extend({
		initialize: function(){
			...
			$.getJSON('../../reader/magazineAssets/colors/'+pageURL+'-colors.json')
			.done(function(data){
	...
	function getEndPage() { //End Pages is the value of the last page in the entire mag
		$.getJSON('../../reader/magazineAssets/data/'+pageURL+'.json')
		.done(function(data){
	...
	var layout = {
		...
		update: function(){
			...
			// Grab the page
			console.log("States: " + states.currentPage);
		...
		lazyLoadImages: function(page){
			...
			for (var i = 0; i < range.length; i++){
				...
				if (src.indexOf('data:image\/gif') > -2) {
					$img.attr('src', '../../reader/magazineAssets/'+pageURL+'/page-'+page_number+'.'+PULP_SETTINGS.imgFormat );
				}
			}
